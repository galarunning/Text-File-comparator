...
#Wait for 2 minute until IP power switch off
Write-Host "$IP_Power_IP - IP Power Switch Powering off in 2 Minute"
Start-Sleep -s 120


#IP Power switch off command
$url = "http://${IP_Power_username}:${IP_Power_password}@$IP_Power_IP/Set.cmd?CMD=SetPower+$Power_Option_2=$Power_off"
cmd /c "curl $url"
...
$url = "http://${IP_Power_username}:${IP_Power_password}@$IP_Power_IP/Set.cmd?CMD=SetPower+$Power_Option_2=$Power_On"
cmd /c "curl $url"
