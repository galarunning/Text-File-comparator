...
Write-Host "$IP_Power_IP - IP Power Switch Powering off in 2 Minutes..."
Start-Sleep -s 120


#IP Power switch off command
$url = "http://${IP_Power_username}:${IP_Power_password}@$IP_Power_IP/Set.cmd?CMD=SetPower+$Power_Option_1=$Power_off"
cmd /c "curl $url"
...
$url = "http://${IP_Power_username}:${IP_Power_password}@$IP_Power_IP/Set.cmd?CMD=SetPower+$Power_Option_1=$Power_On"
cmd /c "curl $url"
Write-Host "$IP_Power_IP - IP Power Switch Powered On complete"

#Wait until RDP is back online
Write-Host "Waiting 30 seconds until RDP is not back online and trying connecting continuously every 15 seconds"
Start-Sleep -s 30
